...
// Usamos la clave secreta de Stripe desde variable de entorno
...
const confirmPayment = async (req: Request, res: Response) => {
  ...
  if (!paymentMethodId || !clientSecret) {
    res
      .status(400)
      .json({
        error: "Faltan datos requeridos (paymentMethodId o clientSecret)",
      });
    return;
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: (error as any).message || "Error al confirmar el pago" });
    return;
  }
};
